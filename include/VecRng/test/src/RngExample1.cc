...
void CheckSum(double theory, double sigma, double sample) 
{
  ...
  if(pass) std::cout << " CheckSum PASSED: sampleSum = " << sample << std::endl;
  else     std::cout << " CheckSum FAILED: sampleSum = " << sample << std::endl;
}
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < vsize ; ++i) srng[i].Initialize();

  // generate scalar random numbers: total of nsample x vsize
  double ssum = 0;
  for (int i = 0; i < nsample ; ++i) {
    for(int i = 0; i < vsize ; ++i) ssum += srng[i].Uniform<ScalarBackend>();
  }
  ...
  vectorRNG::State_t *vstate = new vectorRNG::State_t;
  for (int j = 0; j < vsize ; ++j) {
    for (int i = 0; i < vecRng::cxx::MRG::vsize ; ++i) {
      vstate->fCg[i][j] = srng[j].GetState()->fCg[i];
    }
  }
  vrng->SetState(vstate);
  ...
  ssum = 0;
  for (int i = 0; i < nsample ; ++i) {
    for(int j = 0; j < vsize ; ++j) ssum += srng[j].Uniform<ScalarBackend>();
  }
  CheckSum(mean, sigma, ssum);
